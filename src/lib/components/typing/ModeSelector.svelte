...
<!--
  Mobile: two rows внутри карточки.
    Row 1: [Время] [Слова] ............ [Сложный/Лёгкий]
    Row 2: [15] [30] [60]                 (выровнено по центру)
  Desktop (sm+): всё одной строкой как раньше.
-->
<div class="s-card px-2 py-1.5 w-full sm:w-auto flex flex-col sm:flex-row sm:flex-wrap gap-1.5 sm:gap-1 items-stretch sm:items-center justify-center">

  <!-- Group 1: mode type + (mobile-only) difficulty on the right -->
  <div class="flex items-center gap-1 w-full sm:w-auto">
    ...
    <!-- Spacer + difficulty (mobile only) -->
    <div class="flex-1 sm:hidden"></div>
    <button class="chip-sakha transition-all sm:hidden {settings.difficulty === 'expert' ? 'bg-error-500/10' : 'bg-success-500/10'}"
  ...
  <button class="chip-sakha transition-all hidden sm:inline-flex {settings.difficulty === 'expert' ? 'bg-error-500/10' : 'bg-success-500/10'}"
          class:text-error-400={settings.difficulty === 'expert'}
          class:text-success-400={settings.difficulty !== 'expert'}
          on:click={toggleDifficulty}>
    {settings.difficulty === 'expert' ? 'Сложный' : 'Лёгкий'}
